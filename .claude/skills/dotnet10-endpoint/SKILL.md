...
Backend nằm ở `backend/` (.NET 10 Minimal API, C#). Chạy: `cd backend && dotnet run --urls http://localhost:5155`. Test: `cd backend && dotnet test`.
...
- [ ] `docs/API.md` cập nhật cùng commit; `cd backend && dotnet test` pass.
